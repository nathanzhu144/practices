# Nathan Zhu Sunday July 27th, 2020, Stockton CA.  GOT POSITION #36/14309 for top 0.25% in Weely Contest 199.
...
def sequentialDigits(self, low, high):
    ...
    """
    ret = []
    def helper(curr, currstr, minlen, maxlen):
        if minlen <= len(currstr) <= maxlen: 
            cand = int("".join(currstr))
            #print(cand)
            if low <= cand <= high:
                ret.append(cand)
            
        if len(currstr) > maxlen: return
        
        if curr <= 9: helper(curr + 1, currstr + [str(curr)], minlen, maxlen)
            
    for i in range(1, 9):
        helper(i, [], len(str(low)), len(str(high)))
